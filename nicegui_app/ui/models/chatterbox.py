...
def chatterbox_controls():
    with ui.column().classes("w-full gap-6 p-4 border border-gray-200 rounded-xl"):
        ui.label("Saved Voice Profiles").classes(
            "font-semibold text-gray-700 w-full text-center"
        )
        ui.select(
            options=["Profile 1", "Profile 2", "Default"],
            value="Default",
        ...
        with ui.column().classes("relative w-full"):
            ...
            reference_audio_player_container = ui.row().classes(
                "w-full mt-2 p-2 border border-gray-200 rounded-xl bg-gray-50"
            )
                ...
                uploader = (
                    ui.upload(
                        ...
                        on_upload=lambda e: handle_file_upload(
                            e, uploader_container, reference_audio_player_container
                        ),
